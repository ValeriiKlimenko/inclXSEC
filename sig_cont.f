      double precision FUNCTION sig_cont(Ebeam,Epr,theta)
      IMPLICIT none
      include 'constants8.inc'
      double precision Ebeam,sn2,nu,Epr,q2,theta,peak
      real Es1,Ep1,sn21,vw2,w2,w1,dsig,xmott,x,bres
      integer ityp,ihd
      data ityp/1/,ihd/1/
      dsig=0.0e+0
      nu=Ebeam-Epr
      sn2=0.5d0*(1.d0-cos(theta))
      q2=4.d0*Ebeam*Epr*sn2
      peak=Ebeam/(1.d0+2.d0*Ebeam/mp*sn2)
      if(Epr.lt.peak) then
      Es1=real(Ebeam)
      Ep1=real(Epr)
      sn21=real(sn2)
      x=real(q2/(2.d0*mp*nu))
      call xsechd(Es1,Ep1,sn21,ityp,ihd,vw2,w2,w1,dsig,xmott,x,bres)
      endif
      sig_cont=dble(dsig)
      return
      end
c#################################################################### 	
       SUBROUTINE XSECHD(E0,EP,SINSQ,ITYP,IHD,VW2,W2,W1,XSECTN,XMOTT,
     >X,bres)
C...
...
!   bres is value of modulating function of F2 that
!   accounts for background and resonances               (output)
C...
      DOUBLE PRECISION C(24),CF(11),CD(24),CFD(11),F2,B,QSQ,WW
...
C... TERMS)
C... from Valerii Iteration 3:
      DATA   C(1) / 0.10741163D 01/,  C(2) / 0.75531124D 00/,
     *       C(3) / 0.33506491D 01/,  C(4) / 0.17447015D 01/,
     *       C(5) / 0.35102405D 01/,  C(6) / 0.11439100D 01/,
     *       C(7) / 0.12299128D 01/,  C(8) / 0.11473500D 00/,
     *       C(9) / 0.62197400D 00/,  C(10)/ 0.14982600D 01/,
     *       C(11)/ 0.12269000D 00/,  C(12)/ 0.51489800D 00/,
     *       C(13)/ 0.17118400D 01/,  C(14)/ 0.11770000D 00/,
     *       C(15)/ 0.51329000D 00/,  C(16)/ 0.19434300D 01/,
     *       C(17)/ 0.20270200D 00/,  C(18)/-0.17498537D 00/,
...
C... Valerii Upd
      DATA CF(1) / 0.23670000D 00/,  CF(2) / 0.21780000D 01/,
     *     CF(3) / 0.89800000D 00/,  CF(4) /-0.67260000D 01/,
     *     CF(5) / 0.37180000D 01/,  CF(6) / 0.16421119D 01/,
...
C     Units of cross section are microbarns per steradian per GeV
      CSMOTT=(19.732/(2.0*ALPHAX*E0*SINSQ))**2*COSSQ
      xmott=csmott
      OMEGAP=TPM*V/Q2+PMSQ/Q2
C...
C...OVERCOME RISK OF UNDERFLOW IN THE EXPONENTIATION
C...
      OMEGAP=MIN(20.0,OMEGAP)
C...
...
C...
   3  VW2=UNIV*bres	!to take out the background and resonance
...
C...      DATA PMSQ/.880324D0/,PM2/1.876512D0/,PM/.938256D0/
      DATA NRES/4/,NBKG/5/
C...
C...KINEMATICS
C...
